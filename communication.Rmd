...
alpha <- Cron.bach$total$raw_alpha
alpha.drop <- as.data.frame(Cron.bach$alpha.drop$raw_alpha)
minimum.alpha.drop <- min(alpha.drop )
maximum.alpha.drop <- max(alpha.drop )
```
...
coefficientalpha::tau.test(tau_data )
coefficientalpha::omega(tau_data )
...
decriptives <- cbind(decriptives, alpha.drop )
...
efa.data_re <-  efa.data %>% dplyr::select( CS03, CS04, CS06, CS07,CS08,CS10,CS11, CS12, 
                 CS13,CS14,CS15,CS17, CS18, CS19, CS20 ,CS21, CS22)


#KMO test
KMO <- psych::KMO(efa.data_re) 

# Test of correlation matrix
bartlet <- psych::cortest.bartlett(efa.data_re, n =nrow(efa.data_re))


```

```{r eval=FALSE, gtfig, include=F, include=FALSE}
...
gt.table <- as.data.frame(cbind(gt.table$Items, gt.table$mean, gt.table$sd, decriptives$`Shapiro-Wilk Statistics`, decriptives$`Item-Total Correlation`, decriptives$`Alpha Drop`,gt.table$hist_data, gt.table$dens_data ))
colnames(gt.table) <-  c("Item","Mean", "SD", "SW","CITC", "Alpha Drop",
                         "hist_data", "dens_data")

gt.table$Mean <- as.numeric(gt.table$Mean)
gt.table$SD <- as.numeric(gt.table$SD)
gt.table$`Alpha Drop` <- as.numeric(gt.table$`Alpha Drop`)
...
Table <-  cbind( gt.table, gt.liket )%>% 
  ...
  fmt_number(columns = Mean:SD, decimals = 1) %>%
   fmt_number(columns = `Alpha Drop`, decimals = 2) %>%
  # header
  tab_header(
    title = md("**Communication Scale**"),
    subtitle = md("Summary Descriptives (n=340)")
  ) %>% 
  tab_footnote(
    footnote = md("**Shapiro-Wilk Statistics**"),
    locations = cells_column_labels(columns = SW)
  ) %>%
  tab_footnote(
    footnote = md("**Corrected Item-Total Correlation**"),
    locations = cells_column_labels(columns = CITC)
    )%>% 
  tab_spanner(
    label = "Items",
    columns = Item
...
correlations <- psych::polychoric(efa.data_re)
...
p.mat <- correlation::cor_to_p(correlations$rho, nrow(efa.data), method = "polychoric")
...
ggsave("Figures/600/corplot.png",corplot, width = 3.5, height = 3.5, dpi = 300, bg ="White" )
...
# Parallel analysis
Horn <- paran(efa.data_re , iterations=500, centile=0, quietly=FALSE,
...
Scree <- scree(efa.data_re,factors=TRUE,pc=TRUE,main="(B)",
      hline=NULL,add=F)
...
scree.plot <-ggplot(FA.Scree, aes(x=num, y=eigenvalue,color=tye, shape=type))+
  ...
  scale_x_continuous(name='Factor Number (Scree Plot)', limits=c(0, 10),breaks=seq(0,10,2))+
  geom_hline(yintercept=1, linetype = 'dashed') + apatheme +theme(legend.position = "None") + labs(title = "B")
#text = element_text(size = 25
...
# HULL
EFA.MRFA::hullEFA(efa.data_re,extr = "ULS", index_hull = "CAF", display = TRUE, graph = T,
        details = TRUE)
...
Hull <- hull+ ggtitle(NULL)+xlab("Factor Number (Hull Method)")+ylab("CAF")+ aes(color = '#DC000099')+geom_point(color ='#3C5488FF',size =1)+
  apatheme +theme(legend.position = "None") +scale_fill_identity()+ labs(title = "C")
...
#MAP
map <- psych::VSS(efa.data_re, rotate = "promax", fm = 'pa', n.obs =nrow(efa.data_re))
...
write_csv(full.map, "MAP_stat.csv")
```

```{r map}
papaja::apa_table(full.map, caption = "Minimum average partial (MAP) method of factor numder .")
```
...
library(patchwork)
all_plot_2 <-corplot |(scree.plot/Hull)


ggsave("Figures/600/factors.jpg",all_plot_2 , width = 7, height = 6, dpi = 300)
...
fa.1F.1 <- fa(r=correlations$rho, nfactors = 1, fm= "pa",
              residuals = TRUE, SMC = TRUE)
EE<- print(fa.1F.1, cut = .3, digits = 3, sort = TRUE)

one.factor.alpha <- psych::alpha(correlations$rho ,check.keys=TRUE)
...
```
The initial solution of the fitted one factor model had six items with poor factor loadings (Item 1, 2,5,9, 16, 23). We discarded these items from the model and run another EFA. This iteration of EFA yielded a one factor simple structure with 17 items.   The internal consistency reliability coefficient for this model was, ordinal $\alpha$ = `r apa(one.factor.alpha,2,F)`.
...
knitr::include_graphics("Figures/600/CFAplot.png", dpi =600)
...
range.outfit <-  range(IRT.details$`Standardized Outfit`)
range.infit <-  range(IRT.details$`Standardized Infit`)
...
person <- ggplot(personfit, aes(x=Zh)) + geom_histogram(binwidth=.5,col=I("black"),fill="#00A08799")+geom_vline(xintercept = -2)+
...
ggsave("Figures/600/person.png",person , width = 3.5, height = 3.5, dpi = 300, bg = "white")
...
knitr::include_graphics('Figures/600/person.png')
...
knitr::include_graphics("Figures/600/itemInfoplot.png", dpi =600)
...
test.info <- tabledown::ggtestinfo_se(irt.data.1, fit)+ apatheme+geom_line(colour = "black", size=1.5)+geom_area(fill ="#E64B3599" )+
geom_hline(yintercept = 3.5)+
geom_vline(xintercept = -4.3)+
geom_vline(xintercept = 1.2)+
geom_text(data = tibble (x = -4.7, y = .1 ),
            aes(x=x, y = y,  label = "-4.3"), inherit.aes = FALSE, size = 6)+
geom_text(data = tibble (x = 1.7, y = .1 ),
            aes(x=x, y = y,  label = "1.2"), inherit.aes = FALSE, size = 6)+
geom_text(data = tibble (x = -6, y = 3.6 ),
            aes(x=x, y = y,  label = "3.5"), inherit.aes = FALSE, size = 6)



rel.info <- tabledown::ggreliability(irt.data.1, fit)+ apatheme+geom_line(colour = "black", size=1.5)+geom_area(fill ="#E64B3599" )+
geom_hline(yintercept = 0.70)+
geom_vline(xintercept = -5.3)+
geom_vline(xintercept = 2.3)+
geom_text(data = tibble (x = -5.7, y = .05 ),
            aes(x=x, y = y,  label = "-5.3"), inherit.aes = FALSE, size = 6)+
geom_text(data = tibble (x = 2.7, y = .05 ),
            aes(x=x, y = y,  label = "2.3"), inherit.aes = FALSE, size = 6)+
geom_text(data = tibble (x = -6, y = .72),
            aes(x=x, y = y,  label = "0.70"), inherit.aes = FALSE, size = 6)


# +geom_vline(xintercept = -3, colour ="black")+
#   geom_vline(xintercept = .5, colour ="black")
ggsave("Figures/600/TIC.png",test.info , width = 3.5, height = 3.5, dpi = 300, bg = "white")
...
CSS_total <-  irt.data.1 %>%
  rowwise() %>% 
  mutate(Total_communication = sum(across(starts_with("CS")), na.rm = T))


theta_total_correlation <- corr.test(CSS_total$Total_communication,estimation  )
...
totalscore <- (expected.test(fit , Theta))
totalscore_1 <- as.data.frame((cbind(Theta, totalscore)))

totalscore_2 <- totalscore_1

totalscore_2$totalscore <- round(totalscore_2$totalscore,0)

theta_estimatedtotalscore <- totalscore_2 %>% 
...
ggscore <- ggplot(totalscore_1, aes(x=Theta, y=totalscore))+geom_line()+apatheme+xlab(expression(theta))+ylab(expression(T(theta)))+geom_segment(aes(x=0,xend=-Inf,y=48.4,yend=48.4, color="#4DBBD599"))+
  geom_segment(aes(x=0,xend=0,y=-Inf,yend=48.4, color="#4DBBD599"))+theme(legend.position = "none")+ylab("Percentile Rank") +scale_y_continuous(breaks=perc, labels= l)
  # geom_text(data = tibble (x = -.22, y = 15), 
  #           aes(x=x, y = y,  label = "14.10"), inherit.aes = FALSE, size = 4)


```

```{r irt plots}
tic_rc <- cowplot::plot_grid(person, test.info, rel.info,ggscore,
                   labels = "AUTO",
                   ncol=2, 
                   label_size = 20,
                  align = "h", 
                  label_fontfamily = "sans",
                  label_fontface = "plain")
ggsave("Figures/600/tic_rc.jpg",tic_rc , width = 7, height = 6, dpi = 300)
...
knitr::include_graphics('Figures/600/personmap.jpeg')
...
knitr::include_graphics("Figures/600/TIC.png", dpi =600)

```

```{r figconrel, fig.cap = "Conditional Reliability.", out.width= '100%', out.height='50%', warning=FALSE}

knitr::include_graphics("Figures/600/conrel.png", dpi =600)


```

```{r estimatedscore}
apa_table(theta_estimatedtotalscore, caption="Ability and estimated total score")
